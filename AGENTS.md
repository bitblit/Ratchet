...
- Monorepo managed with pnpm workspaces; packages live in `modules/*` (examples: `common`, `aws`, `node-only`, `ngx-acute-*`).
...
- Lint: `pnpm lint` (or `pnpm lint-fix` to format + fix). Prettier config drives formatting.
- Tests: `pnpm test` runs vitest across packages; coverage `pnpm test-coverage`. Package-only: `cd modules/common && pnpm mod:test`.
...
- Before opening a PR: describe the change, list impacted packages, note dev commands run (`pnpm test`, `pnpm lint`, builds), and link any issues.
