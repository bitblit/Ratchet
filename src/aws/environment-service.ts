...
export class EnvironmentService {
    ...
    public static async getConfigS3(bucketName: string, path: string, region: string = 'us-east-1'): Promise<any> {
    ...
    private static async retryingGetParamsS3(bucketName: string, path: string, region: string = 'us-east-1',
        ...
        if (!!parsedValue) {
            ...
            ErrorRatchet.throwFormattedErr('Could not find system parameter in s3 at %s / %s in this account',
                bucketName, path);
        }

    }

    private static async retryingGetParamsSSM(name: string, region: string = 'us-east-1', ssmEncrypted: boolean = true,
                                           maxRetries: number, backoffMultiplierMS: number):Promise<any> {
