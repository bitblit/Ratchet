...
export class GitRatchet {
    ...
    private static getCommandString(splitChar: string): string {
        return 'git log -1 --pretty=format:"' + GitRatchet.PRETTY_FORMAT.join(splitChar) + '"' +
    ...
    public static async getLastCommit(options:any = {}) : Promise<GitCommitData> {
        ...
        let a: string[] = res.split(GitRatchet.SPLIT_CHARACTER);

        // e.g. master\n or master\nv1.1\n or master\nv1.1\nv1.2\n
        let branchAndTags: string[] = a[a.length - 1].split('\n').filter(n => n);
        let branch: string = branchAndTags[0];
        let tags: string[] = branchAndTags.slice(1);
