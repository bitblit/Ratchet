...
#CURRENT_DATE=`date +"%Y-%m-%dT%H:%M:%S%z"` && find . -wholename "*/lib/*/build/*.js" -type f -exec sed -i -e "s/LOCAL-TIME-ISO/$CURRENT_DATE/g" \
CURRENT_DATE=`date +"%Y-%m-%dT%H:%M:%S%z"` && find . -wholename "*/lib/index.mjs" -type f -exec sed -i -e "s/LOCAL-TIME-ISO/$CURRENT_DATE/g" \
